...
async function main() {
    ...
    logger_1.logger.printFeatures();
    logger_1.logger.separator();
    logger_1.logger.loading('Inicializando sistema avanzado de Baileys');
    await new Promise(resolve => setTimeout(resolve, 1000));
    logger_1.logger.clearLoading();
    logger_1.logger.success('Sistema inicializado correctamente');
    logger_1.logger.loading('Configurando características premium');
    ...
    logger_1.logger.success('Configuración completada - Modo LEGENDARIO activado');
    logger_1.logger.separator();
    logger_1.logger.connection('connecting', 'Estableciendo conexión segura con WhatsApp Web');
    try {
        logger_1.logger.loading('Autenticando credenciales');
        const socket = await soblend.connect('auth_info');
        logger_1.logger.clearLoading();
        logger_1.logger.connection('connected', '¡Conexión establecida! Bot operativo al 100%');
        logger_1.logger.success('Todas las características premium están activas');
        logger_1.logger.separator();
        socket.ev.on('messages.upsert', async ({ messages }) => {
            for (const msg of messages) {
                ...
                logger_1.logger.message(sender, text, 'incoming');
                if (text.toLowerCase() === 'buttons') {
                    ...
                    logger_1.logger.success('Botones interactivos enviados');
                }
                if (text.toLowerCase() === 'list') {
                    logger_1.logger.info('Enviando lista interactiva...');
                    ...
                    logger_1.logger.success('Lista interactiva enviada');
                }
                if (text.toLowerCase() === 'poll') {
                    logger_1.logger.info('Enviando encuesta...');
                    ...
                    logger_1.logger.success('Encuesta enviada');
                }
                if (text.toLowerCase() === 'stats') {
                    const stats = socket.getAdminStats();
                    logger_1.logger.stats(stats);
                    ...
                    logger_1.logger.message(sender, 'Estadísticas enviadas', 'outgoing');
                }
                if (text.toLowerCase() === 'help') {
                    ...
                    logger_1.logger.message(sender, 'Mensaje de ayuda enviado', 'outgoing');
                }
            }
        });
        setInterval(() => {
            const stats = socket.getAdminStats();
            logger_1.logger.table('RESUMEN DE ACTIVIDAD', [
        ...
        logger_1.logger.error(`Error al conectar: ${error}`);
        process.exit(1);
    }
}
main().catch((error) => {
